...
- Now we will install `dotenv` package to be able to import the env file and use it in our config. Open up a terminal pointing at`hardhat-tutorial` directory and execute this command
  ...
  npm install dotenv
  ```
- Now open the hardhat.config.js file, we would add the `Goerli` network here so that we can deploy our contract to Goerli. Replace all the lines in the `hardhar.config.js` file with the given below lines
